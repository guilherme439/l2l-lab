...
class Reporter:
    """Buffers periodic diagnostic snapshots and writes them from a dedicated
    worker thread so callers never block on file I/O or probe-state inference.
    ...
    ``models/{run_name}/reports/``): ``configs/`` (the source YAML archived
    verbatim, plus a stamped copy whenever it changes on resume) and
    ``report_{iter:06d}.md`` snapshots for iterations the caller marks via
    `snapshot_due`.
    """
    ...
    def __init__(
        ...
        self._configs_dir = reports_dir / "configs"
        self._resume = resume

        self._pending_reports: list[StampedReport] = []
        self._starting_iteration: int = 0

        self._requests: Queue[Optional[_SnapshotRequest]] = Queue()
        self._thread = Thread(target=self._run, daemon=True)
        self._thread.start()

    @classmethod
    def clear_artifacts_past(cls, reports_dir: Path, iteration: int) -> None:
        """Remove report snapshots and archived config snapshots past `iteration`."""
        if not reports_dir.exists():
            return
        for path, _ in CommonUtils.find_paths_with_iteration_past(reports_dir, _REPORT_SNAPSHOT_PATTERN, iteration):
            path.unlink()
        for path, _ in CommonUtils.find_paths_with_iteration_past(reports_dir / "configs", _ARCHIVED_CONFIG_PATTERN, iteration):
            path.unlink()
    ...
    def _handle_config_archive(self) -> None:
        dest = self._configs_dir / "config.yaml"
        ...
        archive_path = self._configs_dir / archive_name
    ...
    def _latest_archived_config(self) -> Path:
        ...
        for path in self._configs_dir.iterdir():
        ...
        return self._configs_dir / "config.yaml"
